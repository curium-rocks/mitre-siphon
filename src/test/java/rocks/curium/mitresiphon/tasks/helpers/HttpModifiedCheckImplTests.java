...
class HttpModifiedCheckImplTests {
  ...
  void returnsTrueWhenProvidedNull() throws ExecutionException, InterruptedException, IOException {
    mockServer.enqueue(
        new MockResponse().setBody("").addHeader("last-modified", "Wed, 21 Oct 2015 07:28:00 GMT"));
    String url = String.format("http://localhost:%s/returnsTrueWhenProvided", mockServer.getPort());
  ...
  void returnsFalseIfEqual() throws IOException, ExecutionException, InterruptedException {
    mockServer.enqueue(
        new MockResponse().setBody("").addHeader("last-modified", "Wed, 21 Oct 2015 07:28:00 GMT"));
    String url = String.format("http://localhost:%s/returnsFalseIfEqual", mockServer.getPort());
    HttpModifiedCheck modifiedCheck = new HttpModifiedCheckImpl(WebClient.builder());
    ZonedDateTime lastCheck =
        ZonedDateTime.parse("Wed, 21 Oct 2015 07:28:00 GMT", DateTimeFormatter.RFC_1123_DATE_TIME);
  ...
  void returnsFalseIfOlder() throws IOException, ExecutionException, InterruptedException {
    ...
    HttpModifiedCheck modifiedCheck = new HttpModifiedCheckImpl(WebClient.builder());
    ZonedDateTime lastCheck =
        ZonedDateTime.parse("Wed, 21 Oct 2020 07:28:00 GMT", DateTimeFormatter.RFC_1123_DATE_TIME);
    Assertions.assertFalse(modifiedCheck.isNewDataAvailable(lastCheck, URI.create(url)).get());
  }
