...
/**
 * Interface establishing contract for determining if a http resources has been modified since the
 * last fetch
 */
public interface HttpModifiedCheck {
   ...
   * @return future that resolves with true if there is new data available or false if the data
   *     hasn't changed since last checked
   */
  CompletableFuture<Boolean> isNewDataAvailable(ZonedDateTime lastFetchTime, URI resourceUri)
      throws IOException;
}
