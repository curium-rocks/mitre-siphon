...
public abstract class SimpleExecutionMigration extends BaseJavaMigration {
  ...
  /**
   * Creates a simple execution migration object with the throw behavior on false return set to true
   */
  ...
  /**
   * Creates a simple execution migration with the throw behavior on false return specified as a
   * parameter
   *
   * @param throwOnFalse {boolean} whether the migration should throw and fail if the query
   *     execution method returns false
   */
   ...
   * @throws Exception {Exception} throws if the query is malformed, and depending on configuration,
   *     if the execute method returns false
   */
